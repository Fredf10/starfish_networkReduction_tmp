...
\subsection{The various types of exceptions}
...
Now. What should your test do? Let's go back to \href{http://stackoverflow.com/questions/61400/what-makes-a-good-unit-test}{"A TRIP"} again, and how you fulfil the requirements for this acronym.

\begin{itemize}
\item Automatic. This one is fairly simple, you want to have your tests in a python script which uses the built in "unittest" package. I'll get to that in a while.
\item Thorough. Now, this one is a bit trickier. What you might be thinking is that all you need to do is test the function on top, and if that works, then the test is done. This is not unit testing. Unit testing is to test every single part of the code that can be considered an independent unit. This ties back into the black boxes we talked about earlier. Everything that can be considered a black box, should be tested accordingly.
\item Repeatable. Well, this one really falls under automatic in my opinion. 
\item Independent. As the answer at stackoverflow said, this is the most important one. Every test should test one thing, and one thing alone. If your function should switch between three different outputs depending on its input\footnote{Well, if it does, you really should look into refactoring your function, but this is just for the purpose of demonstration.}, then you should have three separate tests to check that all three work. They should not depend on the result of anything else. This is important because you need to be absolutely sure that something fails or passes solely on its own merit.
\item Professional. Your tests should meet standards of programming. Essentially, your test code should be clean and understandable. Not only because others might need it to be, but because clean code will usually have less bugs. It won't help to test your code if your test doesn't work. 
\end{itemize}

There is one last one that is mentioned, which I find to be absolutely crucial, and that is that a test should be fast. Let's look back at the case I used to illustrate the need for exceptions. A function that would take in 100 files, feed each file into another function. To test that, you only need to feed it two files, that can be empty for all it cares. As a matter of fact, because it asks the moduleFileRead for the actual reading, the test doesn't even have to check that it reads the files, only that it calls moduleFileRead.readFile() with the elements from its input list. Similarly, you only have to test that it then takes whatever output, as long as its not an exception, and passes it to the processing module. 

In other words, the test should test the full, yet minimal, requirement of the functionality of the function, and it should test it well. 
