...
class pettersenAorticBR(AorticBaroreceptor):
    ...
    def __call__(self):
        ...
        self.ResistanceEffector(n_step)

    def calcAndupdatePeriodTypeIIcellML(self, strain):
        super(pettersenAorticBR,self).calcAndupdatePeriodTypeIIcellML(strain)
        n = self.currentTimeStep[0]
        newTotalResistance = self.ratio*(self.R0 + self.delta_TPR[n+1])

        for key in self.boundaryConditionIIout:
            if self.boundaryConditionIIout[key].name == 'Windkessel-3Elements':
                pass # self.boundaryConditionIIout[key].Rtotal = (newTotalResistance)/self.ResTot0*self.Res0[key]

            ## for Resistance outlets - not tested
            elif self.boundaryConditionIIout[key].name == 'Resistance':
                pass #self.boundaryConditionIIout[key].Rc = (newTotalResistance)/self.ResTot0*self.Res0[key]
    ...
    def ResistanceEffector(self,n):
        ...
        # determine "forcing term" with respect to Tsym_min and delay of the effector
        if n <= delay:
            ...
            deltaF = self.Tsym[n+1-delay] - self.Tsym_min
        ...
        self.Rp[n+1] = self.R0 + self.delta_TPR[n+1]
...
#
        ...
        self.pn = 70*133.32 # 92*133.32 # 135*133.32 # adjusted from 92 mmHg as given in paper, chose a value close to the initialisation
...
#         self.Vheart = np.zeros(self.nTsteps+1)
    ...
    def updateBC2(self,n):
        ...
        if self.boundaryConditionII is not None:
            ...
            # self.boundaryConditionII.self.Emin_BRX = self.Emin
